...
```{=html}
<section class="hero">
  <div class="eyebrow">ImmPrint immune gene set collection · v0.4.0-alpha</div>
  ...
  process, kept apart from the ligand that triggers it, placed in an Input →
...
```{ojs}
//| echo: false
data = FileAttachment("data/immprint_0.4.0-alpha.csv").csv()
```
...
heroTiers = [
  { tier: "i", label: "Input", genes: ["IFNGR1", "IFNGR2"] },
  { tier: "t", label: "Transducer", genes: ["JAK1", "JAK2", "STAT1"] },
  { tier: "e", label: "Effector", genes: ["CXCL9", "CXCL10", "GBP1"] }
]
tierColour = ({ i: "var(--tier-input)", t: "var(--tier-transducer)", e: "var(--tier-effector)" })
heroPapers = ({
...
//| echo: false
{
  const el = html`<div class="trace">
    ...
    ${pmidLine(picked)}
    <div class="trace-foot">*These are a few example genes from this set, not the full list.</div>
  </div>`;
...
Every gene is placed in a three-tier Input → Transducer → Effector hierarchy, so a
