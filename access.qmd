...
[Download CSV](data/immprint_0.4.0-alpha.csv){.btn-ghost download="immprint_0.4.0-alpha.csv"}
[Download JSON](data/immprint_0.4.0-alpha.json){.btn-ghost download="immprint_0.4.0-alpha.json"}
...
| `pathway_level` | signalling tier: 1 = Input, 2 = Transducer, 3 = Effector. Tier 1 is the most upstream node *present in the set*: a receptor where the set has one, the driver transcription factor where it does not |
...
| `readout_type` | what a module score over the set means: `inducible` — the pathway ends in transcription, so the effector tier reads out activity; `state` — the set describes a differentiation state, so score all tiers and read the effector tier as phenotype; `machinery` — the score reflects standing apparatus and makes no activity claim |
...
url = "https://akmclz.github.io/immprint/data/immprint_0.4.0-alpha.csv"
ip = pd.read_csv(url)
...
# so split on ";" and do not match the whole string
...
`immprintr(version = "0.4.0-alpha")` and the raw files are version-stamped, so
