...
// Selected-set table: fully displayed, no scroll bar
{
  ...
  return html`<div id="set-detail" style="margin-top:1.8rem">
    ...
    <p class="muted" style="margin:0.2rem 0 0.7rem">${rows.length} genes<span class=${"rtpill " + rows[0].readout_type}>${readoutLabel[rows[0].readout_type] ?? rows[0].readout_type}</span></p>
    ${tableFor(rows)}
...
```{ojs}
tierLabel = ({ "1": "Input", "2": "Transducer", "3": "Effector" })
tierClass = ({ "1": "i", "2": "t", "3": "e" })

readoutLabel = ({ inducible: "Inducible", state: "State", machinery: "Machinery" })
